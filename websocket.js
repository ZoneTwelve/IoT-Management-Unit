/*
Now, here just a demo version for running Socket service
*/
module.exports = function(server){
	require('dotenv').config(); // loading .env config files

	// database connection start
	// REQUIREMENT: need a middleware to access main server data ( like JWT )
	
	// because socket also need write record on database
	// So, create a temporarily connection for demo
	...
	// binding on connection event
	server.on('connection', function( io ){
		// if user want to connect to chatroom, need to sign-in first
		io.on("register", ( data ) => {
